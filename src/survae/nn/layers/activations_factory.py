...
def act_module(act_str, allow_concat=False):
    if allow_concat:
        assert (
            act_str in act_strs + concat_act_strs
        ), "Got invalid activation {}".format(act_str)
    else:
        assert act_str in act_strs, "Got invalid activation {}".format(act_str)
    if act_str == "relu":
        return nn.ReLU()
    elif act_str == "elu":
        return nn.ELU()
        ...
        return Swish()
    elif act_str == "concat_relu":
        return ConcatReLU()
    elif act_str == "concat_elu":
        return ConcatELU()
