...
class MADE(nn.Sequential):
    ...
    def __init__(
        ...
        # Build layers
        data_degrees = MaskedLinear.get_data_degrees(
            features, random_order=random_order, random_seed=random_seed
        )
        ...
        for i, out_features in enumerate(hidden_features):
            layers.append(
                MaskedLinear(
                    ...
                    if random_seed
                    else None,  # Change random seed to get different masks
                    is_output=False,
                )
            )
...
class AgnosticMADE(MADE):
    ...
    def __init__(
        ...
        self.current_mask = 0
        super(AgnosticMADE, self).__init__(
            features=features,
    ...
    def update_masks(self):
        ...
        data_degrees = MaskedLinear.get_data_degrees(
            self.features,
            random_order=self.order_agnostic,
            random_seed=self.current_mask,
        )
        ...
        for module in self.modules():
            if isinstance(module, MaskedLinear):
                module.update_mask_and_degrees(
                    in_degrees=in_degrees,
                    data_degrees=data_degrees,
