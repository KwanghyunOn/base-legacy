...
class DenseTransformer(nn.Module):
    def __init__(
        ...
        decoder_layer = DenseTransformerBlock(
            d_model=d_model,
            nhead=nhead,
    ...
    def _reset_parameters(self):
        ...
        for decoder_layer in self.layers:
            ...
            decoder_layer.self_attn.out_proj.weight.data /= math.sqrt(
                2 * self.num_layers
            )
...
class DenseTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = nn.MultiheadAttention(
            d_model, nhead, dropout=dropout, kdim=kdim, vdim=vdim, bias=attn_bias
        )
    ...
    def _reset_parameters(self):
        ...
        nn.init.normal_(
            self.linear1.weight, std=0.125 / math.sqrt(self.linear1.weight.shape[1])
        )
        nn.init.normal_(
            self.linear2.weight, std=0.125 / math.sqrt(self.linear2.weight.shape[1])
        )
        ...
        nn.init.normal_(
            ...
            std=0.125 / math.sqrt(self.self_attn.in_proj_weight.shape[1]),
        )
        if not self.self_attn._qkv_same_embed_dim:
            nn.init.normal_(
                self.self_attn.q_proj_weight,
                std=0.125 / math.sqrt(self.self_attn.q_proj_weight.shape[1]),
            )
            nn.init.normal_(
                self.self_attn.k_proj_weight,
                std=0.125 / math.sqrt(self.self_attn.k_proj_weight.shape[1]),
            )
            nn.init.normal_(
                self.self_attn.v_proj_weight,
                std=0.125 / math.sqrt(self.self_attn.v_proj_weight.shape[1]),
            )
        ...
        nn.init.normal_(
            ...
            std=0.125 / math.sqrt(self.self_attn.out_proj.weight.shape[1]),
        )
