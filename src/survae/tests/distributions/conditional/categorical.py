...
from survae.distributions import ConditionalCategorical
from survae.nn.layers import LambdaLayer


class ConditionalCategoricalTest(ConditionalDistributionTest):
    def setUp(self):
        self.batch_size = 16
        self.num_classes = 5
        self.size = 10
        ...
        net = nn.Sequential(
            ...
            LambdaLayer(
                lambda x: x.reshape(self.batch_size, self.size, self.num_classes)
            ),
        )
        self.distribution = ConditionalCategorical(net=net)

    def test_distribution_is_well_behaved(self):
        self.assert_distribution_is_well_behaved(
            self.distribution,
            self.x,
            self.context,
            expected_shape=(self.batch_size, self.size),
        )

    def test_logits(self):
        logits = self.distribution.logits(self.context)
        self.assert_tensor_is_good(logits, self.x.shape + (self.num_classes,))

    def test_probs(self):
        probs = self.distribution.probs(self.context)
        self.assert_tensor_is_good(probs, self.x.shape + (self.num_classes,))
    ...
    def test_scalar_output(self):
        x = torch.randint(self.num_classes, [self.batch_size])
        net = nn.Linear(20, self.num_classes)
        distribution = ConditionalCategorical(net=net)
        self.assert_distribution_is_well_behaved(distribution, x, self.context)
