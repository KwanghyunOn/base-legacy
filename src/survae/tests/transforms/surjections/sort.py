...
class SimpleSortTest(SurjectionTest):
    ...
    def test_surjection_is_well_behaved(self):
        self.assert_surjection_is_well_behaved(
    ...
    def test_order_on_tensor_extracted_from_lambd(self):
        self.assertEqual(
            self.x[:, :, 0, 0].sort(1).values, self.transform1(self.x)[0][:, :, 0, 0]
        )
        self.assertEqual(
            self.x[:, 0, :, 0].sort(1).values, self.transform2(self.x)[0][:, 0, :, 0]
        )
        self.assertEqual(
            self.x[:, 0, 0, :].sort(1).values, self.transform3(self.x)[0][:, 0, 0, :]
        )
