...
class SliceTest(SurjectionTest):
    def test_surjection_is_well_behaved(self):
        ...
        surjections = [
            ...
            (
                Slice(
                    ConditionalMeanNormal(nn.Conv2d(7, 1, kernel_size=1)),
                    num_keep=7,
                    dim=1,
                ),
                (7, 4, 4),
            ...
            (
                Slice(
                    ConditionalMeanNormal(nn.Conv2d(4, 4, kernel_size=1)),
                    num_keep=4,
                    dim=1,
                ),
                (4, 4, 4),
            ...
            (
                Slice(
                    ConditionalMeanNormal(nn.Conv2d(1, 7, kernel_size=1)),
                    num_keep=1,
                    dim=1,
                ),
                (1, 4, 4),
        ...
        for surjection, new_shape in surjections:
            with self.subTest(surjection=surjection):
                self.assert_surjection_is_well_behaved(
                    surjection, x, z_shape=(batch_size, *new_shape), z_dtype=x.dtype
                )
