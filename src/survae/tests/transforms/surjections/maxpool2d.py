...
class SimpleMaxPoolSurjection2dTest(SurjectionTest):
    def test_surjection_is_well_behaved(self):
        ...
        surjections = [
            (
                (1, 4, 4),
                SimpleMaxPoolSurjection2d(decoder=StandardHalfNormal((3 * 1, 2, 2))),
                (1, 2, 2),
            ),
            (
                (3, 2, 2),
                SimpleMaxPoolSurjection2d(decoder=StandardHalfNormal((3 * 1, 1, 1))),
                (3, 1, 1),
            ),
            (
                (3, 4, 4),
                SimpleMaxPoolSurjection2d(decoder=StandardHalfNormal((3 * 3, 2, 2))),
                (3, 2, 2),
            ),
            (
                (4, 10, 10),
                SimpleMaxPoolSurjection2d(decoder=StandardHalfNormal((3 * 4, 5, 5))),
                (4, 5, 5),
            ),
        ]
        ...
        for x_shape, surjection, z_shape in surjections:
            with self.subTest(surjection=surjection):
                ...
                self.assert_surjection_is_well_behaved(
                    surjection, x, z_shape=(batch_size, *z_shape), z_dtype=x.dtype
                )
