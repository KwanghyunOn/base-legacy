                ...
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    if num_condition is None:
                        ...
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(
                                    3, 3 * elementwise_params, kernel_size=3, padding=1
                                ),
                        ...
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(
                                    1, 5 * elementwise_params, kernel_size=3, padding=1
                                ),
                                ElementwiseParams2d(elementwise_params),
                            )
                    bijection = GaussianMixtureCouplingBijection(
        ...
        for shape in [(6,), (6, 4, 4)]:
            for num_condition in [None, 1]:
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    bijection = LogisticMixtureCouplingBijection(
                        net, num_mixtures=num_mix, num_condition=num_condition
                    )
                    self.assert_bijection_is_well_behaved(
                        bijection, x, z_shape=(batch_size, *shape)
