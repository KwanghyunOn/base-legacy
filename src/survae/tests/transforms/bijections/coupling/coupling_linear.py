                ...
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    if num_condition is None:
                        ...
                        if len(shape) == 1:
                            net = nn.Linear(1, 5)
                        if len(shape) == 3:
                            net = nn.Conv2d(1, 5, kernel_size=3, padding=1)
                    bijection = AdditiveCouplingBijection(
            ...
            for num_condition in [None, 1]:
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    if num_condition is None:
                        if len(shape) == 1:
                            net = nn.Sequential(
                                nn.Linear(3, 3 * 2), ElementwiseParams(2)
                            )
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(3, 3 * 2, kernel_size=3, padding=1),
                                ElementwiseParams2d(2),
                            )
                    else:
                        if len(shape) == 1:
                            net = nn.Sequential(
                                nn.Linear(1, 5 * 2), ElementwiseParams(2)
                            )
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(1, 5 * 2, kernel_size=3, padding=1),
                                ElementwiseParams2d(2),
                            )
                    bijection = AffineCouplingBijection(
                        net, num_condition=num_condition
                    )
                    self.assert_bijection_is_well_behaved(
                        bijection, x, z_shape=(batch_size, *shape)
