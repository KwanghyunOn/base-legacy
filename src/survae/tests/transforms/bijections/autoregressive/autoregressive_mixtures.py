...
class GaussianMixtureAutoregressiveBijectionTest(BijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        self.eps = 1e-4
        bijection = GaussianMixtureAutoregressiveBijection(
            net, num_mixtures=num_mix, autoregressive_order="ltr"
        )
        self.assert_bijection_is_well_behaved(
            bijection, x, z_shape=(batch_size, features)
        )
...
class LogisticMixtureAutoregressiveBijectionTest(BijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        self.eps = 5e-5
        bijection = LogisticMixtureAutoregressiveBijection(
            net, num_mixtures=num_mix, autoregressive_order="ltr"
        )
        self.assert_bijection_is_well_behaved(
            bijection, x, z_shape=(batch_size, features)
...
class CensoredLogisticMixtureAutoregressiveBijectionTest(BijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        self.eps = 1e-6
        bijection = CensoredLogisticMixtureAutoregressiveBijection(
            net, num_mixtures=num_mix, num_bins=num_bins, autoregressive_order="ltr"
        )
        self.assert_bijection_is_well_behaved(
            bijection, x, z_shape=(batch_size, features)
        )
