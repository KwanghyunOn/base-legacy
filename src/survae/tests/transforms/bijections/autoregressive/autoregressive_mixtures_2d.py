...
class GaussianMixtureAutoregressiveBijection2dTest(BijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        net_spatial = nn.Sequential(
            SpatialMaskedConv2d(
                3, 3 * elementwise_params, kernel_size=3, padding=1, mask_type="A"
            ),
        ...
        self.eps = 1e-4
        bijection = GaussianMixtureAutoregressiveBijection2d(
...
class LogisticMixtureAutoregressiveBijection2dTest(BijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        self.eps = 5e-5
        bijection = LogisticMixtureAutoregressiveBijection2d(
            net, num_mixtures=num_mix, autoregressive_order="raster_cwh"
        )
        self.assert_bijection_is_well_behaved(
            bijection, x, z_shape=(batch_size, *shape)
...
class CensoredLogisticMixtureAutoregressiveBijection2dTest(BijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        self.eps = 1e-6
        bijection = CensoredLogisticMixtureAutoregressiveBijection2d(
            net,
