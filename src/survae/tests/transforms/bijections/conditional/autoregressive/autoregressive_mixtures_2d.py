...
class ConditionalGaussianMixtureAutoregressiveBijection2dTest(ConditionalBijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        net_spatial = CondNet(
            3, context_channels=3, params=3 * num_mix, autoregressive_order="raster_wh"
        )
        net = CondNet(
            3, context_channels=3, params=3 * num_mix, autoregressive_order="raster_cwh"
        )

        self.eps = 1e-4
        bijection = ConditionalGaussianMixtureAutoregressiveBijection2d(
            net, num_mixtures=num_mix, autoregressive_order="raster_cwh"
        )
        ...
        bijection = ConditionalGaussianMixtureAutoregressiveBijection2d(
            net_spatial, num_mixtures=num_mix, autoregressive_order="raster_wh"
        )
        self.assert_bijection_is_well_behaved(
            bijection, x, context, z_shape=(batch_size, *shape)
        )
...
class ConditionalLogisticMixtureAutoregressiveBijection2dTest(ConditionalBijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        self.eps = 5e-5
        bijection = ConditionalLogisticMixtureAutoregressiveBijection2d(
            net, num_mixtures=num_mix, autoregressive_order="raster_cwh"
        )
        self.assert_bijection_is_well_behaved(
            bijection, x, context, z_shape=(batch_size, *shape)
...
class ConditionalCensoredLogisticMixtureAutoregressiveBijection2dTest(
    ...
    def test_bijection_is_well_behaved(self):
        ...
        self.eps = 1e-6
        bijection = ConditionalCensoredLogisticMixtureAutoregressiveBijection2d(
            net,
