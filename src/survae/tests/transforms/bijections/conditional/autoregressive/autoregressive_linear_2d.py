...
class ConditionalAdditiveAutoregressiveBijection2dTest(ConditionalBijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        net_spatial = CondNet(
            3, context_channels=3, params=1, autoregressive_order="raster_wh"
        )
        net = CondNet(
            3, context_channels=3, params=1, autoregressive_order="raster_cwh"
        )

        self.eps = 1e-6
        bijection = ConditionalAdditiveAutoregressiveBijection2d(
...
class ConditionalAffineAutoregressiveBijection2dTest(ConditionalBijectionTest):
    def test_bijection_is_well_behaved(self):
        ...
        context = torch.randn(batch_size, *shape)
        net_spatial = CondNet(
            3, context_channels=3, params=2, autoregressive_order="raster_wh"
        )
        net = CondNet(
            3, context_channels=3, params=2, autoregressive_order="raster_cwh"
        )
        ...
        for autoregressive_order in ["raster_cwh", "raster_wh"]:
            with self.subTest(autoregressive_order=autoregressive_order):
                ...
                bijection = ConditionalAffineAutoregressiveBijection2d(
