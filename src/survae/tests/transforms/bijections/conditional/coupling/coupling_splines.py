            ...
            for num_condition in [None, 1]:
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    if num_condition is None:
                        if len(shape) == 1:
                            net = nn.Sequential(
                                nn.Linear(3 + 6, 3 * num_bins),
                                ElementwiseParams(num_bins),
                            )
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(
                                    3 + 6, 3 * num_bins, kernel_size=3, padding=1
                                ),
                        ...
                        if len(shape) == 1:
                            net = nn.Sequential(
                                nn.Linear(1 + 6, 5 * num_bins),
                                ElementwiseParams(num_bins),
                            )
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(
                                    1 + 6, 5 * num_bins, kernel_size=3, padding=1
                                ),
                                ElementwiseParams2d(num_bins),
                            )
                    bijection = ConditionalLinearSplineCouplingBijection(
            ...
            for num_condition in [None, 1]:
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    if num_condition is None:
                        if len(shape) == 1:
                            net = nn.Sequential(
                                nn.Linear(3 + 6, 3 * num_params),
                                ElementwiseParams(num_params),
                            )
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(
                                    3 + 6, 3 * num_params, kernel_size=3, padding=1
                                ),
                        ...
                        if len(shape) == 1:
                            net = nn.Sequential(
                                nn.Linear(1 + 6, 5 * num_params),
                                ElementwiseParams(num_params),
                            )
                        if len(shape) == 3:
                            net = nn.Sequential(
                                nn.Conv2d(
                                    1 + 6, 5 * num_params, kernel_size=3, padding=1
                                ),
                                ElementwiseParams2d(num_params),
                            )
                    bijection = ConditionalQuadraticSplineCouplingBijection(
                        net, num_bins=num_bins, num_condition=num_condition
                    )
                    self.assert_bijection_is_well_behaved(
                        bijection, x, context, z_shape=(batch_size, *shape)
        ...
        for shape in [(6,), (6, 8, 8)]:
            for num_condition in [None, 1]:
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    bijection = ConditionalCubicSplineCouplingBijection(
                        net, num_bins=num_bins, num_condition=num_condition
                    )
                    self.assert_bijection_is_well_behaved(
                        bijection, x, context, z_shape=(batch_size, *shape)
        ...
        for shape in [(6,), (6, 8, 8)]:
            for num_condition in [None, 1]:
                with self.subTest(shape=shape, num_condition=num_condition):
                    ...
                    bijection = ConditionalRationalQuadraticSplineCouplingBijection(
                        net, num_bins=num_bins, num_condition=num_condition
                    )
                    self.assert_bijection_is_well_behaved(
                        bijection, x, context, z_shape=(batch_size, *shape)
