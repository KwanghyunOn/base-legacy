...
class MADETest(torchtestcase.TorchTestCase):
    def test_shape(self):
        ...
        for random_order, random_mask in [
            (False, False),
            (False, True),
            (True, False),
            (True, True),
        ]:
            with self.subTest(random_order=random_order, random_mask=random_mask):
                model = MADE(
                    features=features,
...
class AgnosticMADETest(torchtestcase.TorchTestCase):
    def test_shape(self):
        ...
        for order_agnostic, connect_agnostic in [
            (False, False),
            (False, True),
            (True, False),
            (True, True),
            ...
            with self.subTest(
                order_agnostic=order_agnostic, connect_agnostic=connect_agnostic
            ):
                model = AgnosticMADE(
                    features=features,
                    num_params=num_params,
