...
import unittest
import copy
from survae.nn.nets.autoregressive import DenseTransformer2d


class DenseTransformer2dTest(torchtestcase.TorchTestCase):
    def test_autoregressive_type_A_raster_cwh(self):
        ...
        x_altered[:, 1, 4, 2] = torch.randint(
            0, 2**num_bits, (batch_size,)
        )  # Alter channel G in position (4,2)

        module = DenseTransformer2d(
            shape,
            output_dim=2,
            num_bits=num_bits,
        ...
        module.out_linear.weight.data = torch.randn_like(
            module.out_linear.weight.data
        )  # Initialized to zero, so changing it to see the output altered
        module.eval()
        y = module(x)
