...
class DecoderOnlyTransformerTest(ModuleTest):
    def test_layer_is_well_behaved(self):
        ...
        module = DecoderOnlyTransformer(
            d_model=features, dim_feedforward=4 * features, nhead=2
        )
        self.assert_layer_is_well_behaved(module, x)

        module = DecoderOnlyTransformer(
            d_model=features,
            dim_feedforward=4 * features,
            nhead=2,
    ...
    def test_autoregressive_type_B(self):
        ...
        module = DecoderOnlyTransformer(
            d_model=features, dim_feedforward=4 * features, nhead=2, dropout=0.0
        )
        module.eval()
        y = module(x)
