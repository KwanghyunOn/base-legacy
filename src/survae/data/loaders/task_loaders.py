...
class PresplitLoader:
    ...
    def get_data_loader(
        self, split, batch_size, shuffle=True, pin_memory=True, num_workers=4
    ):
        return DataLoader(
            getattr(self, split),
    ...
    def get_data_loaders(
        self, batch_size, shuffle=True, pin_memory=True, num_workers=4
    ):
        data_loaders = [
...
class RandomSplitLoader(TrainTestLoader):
    def random_split(self, dataset, test_fraction=0.5, split_seed=0):
        train_idx, test_idx = train_test_split(
            range(len(dataset)), test_size=test_fraction, random_state=split_seed
        )
