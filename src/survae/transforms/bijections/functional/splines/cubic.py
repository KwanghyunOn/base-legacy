...
from survae.transforms.bijections.functional.splines.utils import searchsorted, cbrt

# Adapted from https://github.com/bayesiains/nsf/blob/master/nde/transforms/splines/cubic.py
...
def cubic_spline(
    ...
    ), "Inputs < 0 or > 1. Min: {}, Max: {}".format(
        torch.min(inputs), torch.max(inputs)
    )
