...
def logistic_mixture_transform(
    ...
    def mix_cdf(x):
        return torch.logsumexp(
            log_weights + logistic_log_cdf(x.unsqueeze(-1), means, log_scales), dim=-1
        ).exp()

    def mix_log_pdf(x):
        return torch.logsumexp(
            log_weights + logistic_log_pdf(x.unsqueeze(-1), means, log_scales), dim=-1
        )
