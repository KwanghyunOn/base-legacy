...
from survae.transforms.bijections.functional.mixtures.utils_logistic_censored import (
    cmol_cdf,
    cmol_log_pdf,
)


def censored_logistic_mixture_transform(
    inputs,
    logit_weights,
    means,
    log_scales,
    ...
    def mix_cdf(x):
        return torch.sum(
            log_weights.exp() * cmol_cdf(x, means, log_scales, num_bins), dim=-1
        )

    def mix_log_pdf(x):
        return torch.logsumexp(
            log_weights + cmol_log_pdf(x, means, log_scales, num_bins), dim=-1
        )
