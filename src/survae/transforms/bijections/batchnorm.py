...
class _BatchNormBijection(Bijection):
    ...
    def inverse(self, z):
        if self.training:
            raise RuntimeError(
                "BatchNorm inverse is only available in eval mode, not in training mode."
            )
        return (
            torch.sqrt(self.running_var + self.epsilon)
            * ((z - self.bias) / self.weight)
            + self.running_mean
        )
...
class BatchNormBijection(_BatchNormBijection):
    ...
    def register_params(self):
        ...
        self.register_parameter(
            "log_weight", nn.Parameter(torch.zeros(1, self.num_features))
        )
...
class BatchNormBijection1d(_BatchNormBijection):
    ...
    def register_params(self):
        ...
        self.register_parameter(
            "bias", nn.Parameter(torch.zeros(1, self.num_features, 1))
        )
        self.register_parameter(
            "log_weight", nn.Parameter(torch.zeros(1, self.num_features, 1))
        )
...
class BatchNormBijection2d(_BatchNormBijection):
    ...
    def register_params(self):
        ...
        self.register_parameter(
            "bias", nn.Parameter(torch.zeros(1, self.num_features, 1, 1))
        )
        self.register_parameter(
            "log_weight", nn.Parameter(torch.zeros(1, self.num_features, 1, 1))
        )
