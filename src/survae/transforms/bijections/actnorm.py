...
class ActNormBijection1d(_ActNormBijection):
    ...
    def register_params(self):
        """Register parameters shift and log_scale"""
        self.register_parameter(
            "shift", nn.Parameter(torch.zeros(1, self.num_features, 1))
        )
        self.register_parameter(
            "log_scale", nn.Parameter(torch.zeros(1, self.num_features, 1))
        )
...
class ActNormBijection2d(_ActNormBijection):
    ...
    def register_params(self):
        """Register parameters shift and log_scale"""
        self.register_parameter(
            "shift", nn.Parameter(torch.zeros(1, self.num_features, 1, 1))
        )
        self.register_parameter(
            "log_scale", nn.Parameter(torch.zeros(1, self.num_features, 1, 1))
        )
