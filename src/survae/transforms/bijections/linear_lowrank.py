...
class LinearLowRank(Bijection):
    ...
    def __init__(self, num_features, rank, bias=False):
        ...
        assert (
            rank >= 1 and rank <= num_features
        ), "rank should be 1 <= rank <= num_features, but got rank {}".format(rank)
    ...
    def forward(self, x):
        ...
        """
        z = self.d * x + torch.einsum(
            "dr,br->bd", self.U, torch.einsum("rd,bd->br", self.V, x)
        )
        if self.bias is not None:
            z = z + self.bias
