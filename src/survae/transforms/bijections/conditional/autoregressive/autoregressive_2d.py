...
class ConditionalAutoregressiveBijection2d(ConditionalBijection):
    ...
    def __init__(self, autoregressive_net, autoregressive_order="raster_cwh"):
        ...
        assert isinstance(autoregressive_order, str) or isinstance(
            autoregressive_order, Iterable
        )
    ...
    def inverse(self, z, context):
        with torch.no_grad():
            if self.autoregressive_order == "raster_cwh":
                return self._inverse_raster_cwh(z, context=context)
            if self.autoregressive_order == "raster_wh":
                return self._inverse_raster_wh(z, context=context)
        ...
        for h in range(x.shape[2]):
            for w in range(x.shape[3]):
                for c in range(x.shape[1]):
                    elementwise_params = self.autoregressive_net(x, context=context)
                    x[:, c, h, w] = self._elementwise_inverse(
                        z[:, c, h, w], elementwise_params[:, c, h, w]
                    )
        return x
    ...
    def _inverse_raster_wh(self, z, context):
        ...
        for h in range(x.shape[2]):
            for w in range(x.shape[3]):
                elementwise_params = self.autoregressive_net(x, context=context)
                x[:, :, h, w] = self._elementwise_inverse(
                    z[:, :, h, w], elementwise_params[:, :, h, w]
                )
        return x
