...
class ScalarAffineBijection(Bijection):
    ...
    def __init__(self, shift=None, scale=None):
        ...
        assert (
            isinstance(shift, float) or shift is None
        ), "shift must be a float or None"
        assert (
            isinstance(scale, float) or scale is None
        ), "scale must be a float or None"
    ...
    def forward(self, x):
        ...
        ldj = torch.full(
            [batch_size], self._log_scale * num_dims, device=x.device, dtype=x.dtype
        )
        return z, ldj
