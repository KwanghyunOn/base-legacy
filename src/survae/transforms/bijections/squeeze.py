...
class Squeeze2d(Bijection):
    ...
    def _squeeze(self, x):
        assert len(x.shape) == 4, "Dimension should be 4, but was {}".format(
            len(x.shape)
        )
        batch_size, c, h, w = x.shape
        assert h % self.factor == 0, "h = {} not multiplicative of {}".format(
            h, self.factor
        )
        assert w % self.factor == 0, "w = {} not multiplicative of {}".format(
            w, self.factor
        )
        t = x.view(
    ...
    def _unsqueeze(self, z):
        assert len(z.shape) == 4, "Dimension should be 4, but was {}".format(
            len(z.shape)
        )
        batch_size, c, h, w = z.shape
        assert c % (self.factor**2) == 0, "c = {} not multiplicative of {}".format(
            c, self.factor**2
        )
        if not self.ordered:
            t = z.view(
            ...
            t = z.view(
                batch_size, self.factor, self.factor, c // self.factor**2, h, w
            )
