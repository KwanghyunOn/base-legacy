...
def iwbo_nats(model, x, k, kbs=None):
    ...
    if kbs:
        return -iwbo_batched(model, x, k, kbs).mean()
    else:
        return -iwbo(model, x, k).mean()


def iwbo_bpd(model, x, k, kbs=None):
    """Compute the IWBO in bits per dim."""
    if kbs:
        return -iwbo_batched(model, x, k, kbs).sum() / (x.numel() * math.log(2))
    else:
        return -iwbo(model, x, k).sum() / (x.numel() * math.log(2))
...
def dataset_elbo_nats(model, data_loader, device, double=False, verbose=True):
    with torch.no_grad():
        ...
        for i, x in enumerate(data_loader):
            ...
            count += len(x)
            if verbose:
                print("{}/{}".format(i + 1, len(data_loader)), nats / count, end="\r")
    return nats / count
...
def dataset_iwbo_nats(
    model, data_loader, k, device, double=False, kbs=None, verbose=True
):
    with torch.no_grad():
        nats = 0.0
        count = 0
